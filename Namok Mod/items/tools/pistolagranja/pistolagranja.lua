...
PistolaGranja = WeaponAbility:new()

function PistolaGranja:init()
  ...
  self.validMonsters = {
    mooshi = "mooshispawner",
    mooshibaby = "mooshispawnerbaby",
    hen = "henspawner",
    henbaby = "henspawnerbaby"
  }
  ...
  message.setHandler("confirmRelocate", function(_,_, monsterId, petInfo)
    local mnstype = world.monsterType(monsterId)
    if #storage.storedMonsters < self.maxStorage and (self.weapon.currentState == nil or self.weapon.currentState == self.scan) and (self.validMonsters[mnstype]) then
      ...
      self:dropTresaure(mnstype)      

      local last = #storage.storedMonsters
      table.remove(storage.storedMonsters, last)
...
function PistolaGranja:update(dt, fireMode, shiftHeld)
...
function PistolaGranja:scan()
  ...
  while self.fireMode == "primary" do
    ...
    for _,monsterId in ipairs(monsters) do
      if not promises[monsterId]  and (self.validMonsters[world.monsterType(monsterId)]) then
...
function PistolaGranja:absorb(entityId, monster)
  animator.stopAllSounds("scanning")
...
function PistolaGranja:drawBeam(endPos, didCollide)
...
function PistolaGranja:beamPosition(aimPosition)
...
function PistolaGranja:firePosition()
  return vec2.add(mcontroller.position(), activeItem.handPosition(self.weapon.muzzleOffset))
end

function PistolaGranja:aimVector(inaccuracy)
  local aimVector = vec2.rotate({1, 0}, self.weapon.aimAngle + sb.nrand(inaccuracy, 0))
  aimVector[1] = aimVector[1] * mcontroller.facingDirection()
  return aimVector
end

function PistolaGranja:uninit()
  self:reset()
end

function PistolaGranja:reset()
  animator.stopAllSounds("loop")
  self.weapon:setDamage()
  activeItem.setScriptedAnimationParameter("chains", {})
end

function PistolaGranja:dropTresaure(mtype)  
  sb.logInfo("Animal capturado: %s", mtype)
  
  local treasurePool = self.validMonsters[mtype]
  if treasurePool then
    world.spawnTreasure(mcontroller.position(), treasurePool, world.threatLevel())
    return true
  else
    return false
  end
